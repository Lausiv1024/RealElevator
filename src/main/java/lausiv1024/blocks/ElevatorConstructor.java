...
public class ElevatorConstructor extends ContainerBlock {
    ...
    private void createJamb(Direction direction, BlockPos offset, World world, int yPos){
        for (int i = 0; i < 3; i++){
            ...
            BlockState state = i == 2 ? REBlocks.JAMB_OAK.get().defaultBlockState().setValue(Jamb.HAS_ROOF, true).setValue(FACING, direction) :
                    REBlocks.JAMB_OAK.get().defaultBlockState().setValue(FACING, direction);
